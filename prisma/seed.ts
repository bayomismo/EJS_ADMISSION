...
async function main() {
  ...
  // Override the branding logo/favicon to the real assets (idempotent)
  const brandingRow = await db.setting.findUnique({ where: { key: SETTING_KEYS.branding } });
  if (brandingRow) {
    const v = JSON.parse(brandingRow.value);
    v.logoUrl = "/ejs-logo.png";
    v.faviconUrl = "/ejs-logo.png";
    await db.setting.update({ where: { key: SETTING_KEYS.branding }, data: { value: JSON.stringify(v) } });
  }

  // Application terms pages — required by /apply and /apply/teacher (TermsGate)
...
2. الخضوع لبرنامج التدريب التأهيلي على منهجية توكاتسو.
3. الالتزام بأنظمة المدرسة وقواعد السلوك الوظيفي.
4. للوزارة الحق في إنهاء العقد في حال الإخلال الجسيم بالواجبات.

**سادساً: إقرار المتقدم**
بموافقتي على هذه الشروط، أقر بصحة البيانات المقدمة وأوافق على الخضوع لكافة إجراءات التقييم والاختبار.`;

  const termsPages = [
    { slug: "student-terms", titleAr: "شروط وأحكام التقديم للطلاب", bodyAr: studentTermsBody },
    { slug: "teacher-terms", titleAr: "شروط وأحكام التقديم للمعلمين", bodyAr: teacherTermsBody },
  ];
  for (const p of termsPages) {
    await db.page.upsert({
      where: { slug: p.slug },
      create: { ...p, isActive: true },
      update: { titleAr: p.titleAr, bodyAr: p.bodyAr, isActive: true },
    });
  }
